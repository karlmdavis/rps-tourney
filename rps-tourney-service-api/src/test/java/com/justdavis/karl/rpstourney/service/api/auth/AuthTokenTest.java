...
public final class AuthTokenTest {
	...
	public void jaxbMarshalling() throws JAXBException, XPathExpressionException {
		...
		Node tokenNode = (Node) xpath.evaluate("/rps:authToken", domResult.getNode(), XPathConstants.NODE);
		Assert.assertNotNull(tokenNode);
		Node accountNode = (Node) xpath.evaluate("/rps:authToken/rps:account", domResult.getNode(),
				XPathConstants.NODE);
		...
		Node timestampNode = (Node) xpath.evaluate("/rps:authToken/rps:createdTimestamp", domResult.getNode(),
				XPathConstants.NODE);
		Assert.assertEquals(DateTimeFormatter.ISO_INSTANT.format(authToken.getCreatedTimestamp()),
	...
	public void jaxbUnmarshalling() throws JAXBException, XPathExpressionException {
		...
		URL sourceXmlUrl = Thread.currentThread().getContextClassLoader().getResource("sample-xml/authToken-1.xml");

		// Parse the XML to an object.
		AuthToken parsedToken = (AuthToken) unmarshaller.unmarshal(sourceXmlUrl);

		// Verify the results.
		Assert.assertNotNull(parsedToken);
		Assert.assertEquals(42, parsedToken.getAccount().getId());
		Assert.assertEquals("f211aae3-c46f-47da-ae6c-445f5281c4ee", parsedToken.getToken().toString());
		Assert.assertEquals(Instant.parse("2007-12-03T10:15:30Z"), parsedToken.getCreatedTimestamp());
	}
}
