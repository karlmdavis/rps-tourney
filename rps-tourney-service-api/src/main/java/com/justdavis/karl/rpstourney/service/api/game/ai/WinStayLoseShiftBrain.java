...
/**
 * This {@link IPositronicBrain} employs a "win-stay, lose-shift" strategy, per the following article:
 * <a href= "http://arstechnica.com/science/2014/05/win-at-rock-paper-scissors-by-knowing-thy-opponent/" >Scientists
 * find a winning strategy for rock-paper-scissors</a>. Note that this <strong>isn't</strong> the strategy that is
 * referred to as "the best way to win"; it's the strategy that other, best way is designed to beat. Instead, this
 * strategy is analogous to how most humans play.
 */
public final class WinStayLoseShiftBrain implements IPositronicBrain {
	 ...
	 * implementations, as some of them will want to "borrow" this strategy. As an added convenience, it also lets users
	 ...
	 *            (see {@link #calculateNextThrow(GameView, PlayerRole)})
	 * @param roundIndex
	 *            the {@link GameRound#getRoundIndex()} of the {@link GameRound} to calculate a Throw for (must be less
	 *            than or equal to the actual current round in the specified {@link GameView})
	...
	static Throw calculateThrowForRound(GameView game, PlayerRole role, int roundIndex) {
		...
		/*
		 * The strategy for other rounds is dependent on whether or not the last round's Throw won.
		 */
