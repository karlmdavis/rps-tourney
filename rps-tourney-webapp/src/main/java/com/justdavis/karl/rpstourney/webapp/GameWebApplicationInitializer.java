...
public final class GameWebApplicationInitializer implements WebApplicationInitializer {
	...
	/*
	 * A bunch of Spring documentation mentions that it's important that the filters from {@link
	 * SecurityWebApplicationInitializer} be configured before anything else, thus the {@link Order} annotation here.
	 */

	/**
	 * The web application context-wide initialization parameter that specifies the Spring parent
	 * {@link ApplicationContext} instance that should be used (if any). This is mostly intended for use by integration
	 * tests.
	 *
	...
	/**
	 * The {@link ServletContext#getAttribute(String)} key that the Spring {@link ApplicationContext} will be saved
	 * under. This is intended for use by certain integration tests that want to allow the container to initialize the
	 * Spring context on its own.
	 *
	...
	public void onStartup(ServletContext servletContext) throws ServletException {
		 ...
		 * we'll configure things ourselves. It's expected that some integration tests that use Jetty as the web
		 * application container will provide a parent ApplicationContext. (See
		 * ccom.justdavis.karl.rpstourney.webapp.SpringITConfigWithJetty for more details.)
		 */
		...
		/*
		 * Save the Spring context as a ServletContext attribute. It's expected that some integration tests will allow
		 * Jetty to start "clean" and will need to grab the Spring context out from it once it's running. See
		 * EmbeddedJettySpringContextRule for details.
		 */
		...
		/*
		 * Note: Getting the servlet mappings here correct took forever. The following discussion proved to be very
		 * helpful: http://www.jroller.com /kenwdelong/entry/spring_default_servlets_and_serving. Please also see the
		 * EmbeddedServer class, which provides the JSP and default servlets required by this application. Other
		 * containers that might be used, e.g. Tomcat, would also have to provide these somewhere.
		 */
