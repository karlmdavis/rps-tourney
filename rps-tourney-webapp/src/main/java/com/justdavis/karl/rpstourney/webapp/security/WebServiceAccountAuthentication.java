...
/**
 * An {@link Authentication} implementation that can be used with the web service's {@link Account} class.
 */
public final class WebServiceAccountAuthentication extends AbstractAuthenticationToken {
	 ...
	 *            the already-authenticated guest {@link Account} that this new {@link WebServiceAccountAuthentication}
	 ...
	 *            the already-authenticated guest {@link Account} that this new {@link WebServiceAccountAuthentication}
	 *            instance will represent
	 * @return a {@link List} of {@link GrantedAuthority}s built from the specified {@link Account}'s
	 *         {@link Account#getRoles()}
	 */
	...
	public Object getCredentials() {
		/*
		 * We could maybe also collect an ILoginIdentity in this class' constructor and return that here. However, there
		 * isn't really any need for that now (many other Authentication implementations just return null here), so we
		 * don't bother.
		 */
		return null;
	}
