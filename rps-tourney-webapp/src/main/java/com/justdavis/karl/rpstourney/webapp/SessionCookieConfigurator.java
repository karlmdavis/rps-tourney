...
/**
 * This class is responsible for configuring the application's {@link SessionCookieConfig}. Without this configuration,
 * the <code>JSESSIONID</code> cookies produced by this application will have the wrong <code>Domain</code> and
 * <code>Path</code> attributes when running behind a proxy, which will effectively prevent sessions from working
 * correctly.
 */
public final class SessionCookieConfigurator {
	 ...
	 * </p>
	 * <p>
	 * Marked as non-public and deprecated: this constructor is only intended for use in unit tests.
	 * </p>
	 *
	 * @param springConfig
	 *            the Spring {@link Configuration} class to locate the {@link IConfigLoader} bean from
	 */
	@Deprecated
	...
	/**
	 * Properly configure the {@link ServletContext}'s {@link SessionCookieConfig}.
	 */
	public void applyConfiguration(ServletContext servletContext) {
		/*
		 * We log our entry and exit to this method at 'info', so that the log makes more sense. The Spring startup
		 * involved here will log a bunch of stuff at 'info', and would be pretty confusing without some "bookmarks".
		 */
	...
	/**
	 * @return a Spring {@link AbstractApplicationContext} that can be used to retrieve the application's
	 *         {@link IConfigLoader}
	 */
	private AbstractApplicationContext getSpringContext() {
		/*
		 * There does not seem to be a point in this web application's lifecycle where both the Spring context and
		 * ServletContext are available. ServletContextListeners are one likely option, but apparently they aren't
		 * allowed to modify the ServletContext after it's initialized. So as an alternative, we create a small
		 * "bootstrap" Spring context here, which will only be used to load the AppConfig, and is then disposed of.
		 */
